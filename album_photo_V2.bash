...
if [ -e "$img" ];then
    echo "Album" $img "déjà existant que souhaitez-vous faire ?"
    echo "Supprimer et recreer l'album : 1       Ouvrir l'existant : 0"
    read reponse

    if [ "$reponse" != "0" -a "$reponse" != "1" ];then
        echo "Choix différent de ceux proposés !"
        echo "Error !"
    elif [ "$reponse" == '1' ];then
        echo `rm -r $img`
    else
